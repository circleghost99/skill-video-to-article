...
2. 對 nice_to_have 列表逐一判斷：
   ...
   - 決定不加 → 跳過
3. **不需要載入整份 transcript**，所有 fetch 都按行號 on-demand
4. 進入 Step 08 publish
...
4. 用 Discord embed 分段預覽文章內容（每段一張卡片）
5. 用 Discord embed 預覽每張配圖（含 alt text）
