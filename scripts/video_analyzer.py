...
# ---------------------------------------------------------------------------
ANALYSIS_PROMPT = """\
你是影片視覺分析專家。**只看畫面**（不分析音訊），找出值得截圖或 GIF 的關鍵畫面，寧多勿少。
...
4. 【去重】同一畫面/時段只能出現在 key_frames OR gif_segments，不可兩邊都放。

5. 【欄位約束】
   - `description` ≤ 60 字（中文）。
   - `article_context` 必填，說明素材在文章中的用途。
